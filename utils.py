# utilities for converting openai_harmony conversation model <-> markdown

"""
conversation_md.py (renamed utils.py)

Provides two public helpers:
- conversation_to_markdown(messages)   -> str
- markdown_to_conversation(md_text, message_class=_BaseMessage) -> List[Message]

The markdown format uses an HTML comment at the start of each message to store
metadata (role, optional name).  The rest of the block is free‑form markdown.

Example output:

    <!--msg role=user name=alice -->
    Hello **Alice**! How can I help you today?

    <!--msg role=assistant -->
    Sure – here’s the answer in markdown format.

The comment is invisible to normal markdown renderers but provides a reliable
anchor for parsing.
"""
